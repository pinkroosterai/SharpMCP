...
internal static class CodeSmellChecks
{
    ...
    private static bool ShouldAnalyzeType(INamedTypeSymbol type)
    {
        ...
        if (AnalysisService.HasExcludedAttribute(type))
            return false;
        if (IsAutoGenerated(type))
            return false;
        return true;
    }

    private static bool IsAutoGenerated(INamedTypeSymbol type)
    {
        if (type.GetAttributes().Any(a =>
        {
            var name = a.AttributeClass?.Name;
            return name is "GeneratedCodeAttribute" or "GeneratedCode"
                or "CompilerGeneratedAttribute" or "CompilerGenerated";
        }))
            return true;

        var filePath = type.Locations.FirstOrDefault(l => l.IsInSource)?.SourceTree?.FilePath;
        return IsAutoGeneratedFile(filePath);
    }
        ...
        Compilation compilation, Solution solution, Project project, string solutionDir)
    {
        ...
        {
            if (IsAutoGeneratedFile(document.FilePath))
                continue;
